...
export const createClassSubject = async (
  ...
  const newClassSubject = new ClassSubjectModel({
    teacherId,
    classId,
    subject,
  });
...
export const updateClassSubject = async (
  id: string,
  updatedData: { teacherId?: string; classId?: string; subject?: string }
) => {
  return await ClassSubjectModel.findByIdAndUpdate(id, updatedData, {
    new: true,
  })
